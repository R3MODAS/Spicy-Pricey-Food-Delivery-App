...
const LocationSidebar = () => {
    ...
    return (
        <>
            <div
                className={`location-sidebar fixed top-0 h-full overflow-y-scroll bg-white transition-all duration-500 z-20 sm:px-20 px-5 py-5 w-full sm:py-10 flex flex-col sm:w-[500px] ${isLocationSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
            >
                    ...
                    <ul className="dropdown absolute left-0 right-0">
                        {SearchText &&
                            Locations?.map(item => (
                                <li
                                    onClick={() =>
                                        handleUserLocation(item?.place_id)
                                    }
                                    key={item?.place_id}
                                ...
                                >
                                    <div className="md:p-6 py-4 flex location">
                                        ...
                                        <div className="flex flex-col">
                                            <h3 className="text-base font-ProximaNovaMed text-color-1">
                                                {
                                                    item?.structured_formatting
                                                        ?.main_text
                                                }
                                            </h3>
                                            <h4 className="text-[13px] text-color-5 leading-5 font-ProximaNovaThin">
                                                {
                                                    item?.structured_formatting
                                                        ?.secondary_text
                                                }
            ...
            <div
                className={`location-sidebar-overlay ${isLocationSidebarOpen ? "fixed" : "hidden"} z-10 top-0 left-0 right-0 bottom-0 bg-color-1 opacity-[0.7] overflow-hidden`}
            ></div>
        </>
    );
};
