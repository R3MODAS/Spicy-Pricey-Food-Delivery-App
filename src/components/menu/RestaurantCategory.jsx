...
const RestaurantCategory = ({
    ...
    const handleAddItem = item => {
        ...
        const isSameRestaurantInCart = resInfo?.name === ResInfoData?.name;
        const newItem = { ...item?.card?.info, quantity: 1 };
        if (isItemInCart) {
            toast.error("Already added to the Cart");
        } else {
            if (isSameRestaurantInCart || Object.keys(resInfo).length === 0) {
                toast.success("Added to the Cart");
                dispatch(addToCart({ item: newItem, ResInfoData }));
                setShowPopup(false);
            } else {
                setShowPopup(true);
            }
                ...
                <>
                    ...
                    {ShowPopup && (
                        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 -translate-y-1/3 p-5 bg-white flex flex-col w-full max-w-[500px] h-52 shadow-md popup-anim z-10">
                            <h3 className="font-ProximaNovaSemiBold text-xl">
                            ...
                            <div className="flex justify-between sm:justify-center font-ProximaNovaSemiBold sm:text-sm text-xs sm:gap-0 gap-6">
                                <button
                                    ...
                                    className="bg-white border border-color-11 sm:w-2/5 w-40 sm:h-14 sm:m-5 px-1 py-3"
                                >
                                    NO
                                </button>
                                <button
                                    ...
                                    className="sm:w-2/5 w-40 sm:h-14 sm:m-5 bg-color-11 text-white"
                                >
                                    YES, START AFRESH
                                </button>
                            </div>
